...
const { getAuthConfig, isDevEchoEnabled } = require('../config/env');
  ...
  try {
    if (type === 'email') {
      ...
      try {
        ...
        if (isDevEchoEnabled()) {
          console.warn(`[DEV OTP ECHO] Email send failed (${emailErr.message}). Dev OTP code for ${identifier} is: ${otp}`);
        } else {
          throw emailErr;
        }
      }
